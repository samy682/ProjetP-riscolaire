        ...
        <table class="results">
            ...
            @foreach ($votes as $vote)
                ...
                <tr class="hidden-print">
                    ...
                    <td class="btn-edit"><button type="submit" id="btn" class="btn btn-success btn-xs" name="save" value="{{ $vote->id }}" title="@lang('poll_results.Save the choices') {$vote->name }}">@lang('generic.Save')</button></td>
            ...
            @if (!$hidden)
                ...
                @if ($max > 0)
                    ...
                    <?php $count_best = 0;
                     ...
                     ?>
                    <?php $max = max($best_choices['y']); ?>
                    <?php $nbEnfant =  $poll->nbEnfant; ?>
                    <?php $dateJour =  date("d/m/Y"); ?>
                    <?php    $dateFin = date("d/m/Y",strtotime($poll->end_date));       ?>
                    <tr>
                    @if ($jour == "lun" || $jour == "mar" || $jour == "jeu" || $jour == "ven" || $jour == "sam")
                        <td>Place restante ?</td>
                        @foreach ($best_choices['y'] as $i=>$best_moment)
                            @if ($best_moment % $nbEnfant == 0)
                                <?php $count_best++;
                                $nbLigne++;
                                $nbComplet++; ?>
                                <td><span class="yes-count">Complet</span> @if ($best_choices['inb'][$i]>0) <br/><span class="small text-muted">(+<span class="inb-count">{{ $best_choices['inb'][$i] }}</span>)</span> @endif </td>
                            @elseif ($best_moment%$nbEnfant > 0)
                            <?php 
                                $nbLigne++; ?>
                                <td><span class="yes-count">Il reste encore : <br>{{$nbEnfant - ($best_moment % $nbEnfant)}} place(s)</span>@if ($best_choices['inb'][$i]>0)<br/><span class="small text-muted">(+<span class="inb-count">{{ $best_choices['inb'][$i] }}</span>)</span> @endif </td>
                            @elseif ($best_choices['inb'][$i]>0)
                                <td><br/><span class="small text-muted">(+<span class="inb-count">{{ $best_choices['inb'][$i] }}</span>)</span></td>
                            @else
                                <td></td>
